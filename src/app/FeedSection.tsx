  ...
  if (loading && disputes.length === 0) {
    return (
      <div className="py-8 text-center">
        <div className="animate-pulse-slow text-muted text-sm">
          Loading cases...
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1 rounded-full border border-card-border bg-card-bg p-0.5">
          <button
      ...
      <div className="grid gap-3">
        {disputes.map((d) => (
          <a
            key={d.id}
            href={`/dispute/${d.id}`}
            className="group flex flex-col justify-between rounded-xl border border-card-border bg-card-bg p-4 transition-all hover:border-accent/40 hover:shadow-md active:scale-[0.99]"
          >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 mt-5">
          <button
            ...
            disabled={page === 1}
            className="rounded-full border border-card-border px-4 py-1.5 text-xs font-medium transition-colors hover:border-accent disabled:opacity-30 disabled:cursor-not-allowed"
          >
            Previous
          </button>
